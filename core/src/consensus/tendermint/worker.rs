...
impl Worker {
                    ...
                    recv(receiver) -> msg => {
                        match msg {
                            ...
                            Ok(Event::VerifyPriorityInfo {
                                ...
                                result.send(inner.verify_priority_info(height, priority_info)).unwrap();
                            }
                            Err(crossbeam::RecvError) => {
    ...
    fn is_signer_highest(&mut self, parent_hash: &BlockHash) -> Option<PriorityInfo> {
        self.votes.get_highest_priority_info(self.current_sortition_round()).filter(|priority_info| {
    ...
    fn move_to_step(&mut self, state: TendermintState, is_restoring: bool) {
        ...
        match state.to_step() {
            Step::Propose => {
                ...
                if let Some(hash) = self.votes.get_highest_proposal_hash(vote_step.into()) {
                ...
                if let Some(priority_info) = self.signer_priority_info(parent_block_hash) {
                    if let TwoThirdsMajority::Lock(lock_view, locked_block_hash) = self.last_two_thirds_majority {
                        ...
                        self.votes.collect_priority(self.current_sortition_round(), priority_info.clone());
    ...
    fn on_imported_proposal(&mut self, proposal: &Header) {
        ...
        if proposal_is_for_current {
            if let Some((priority_info, imported_block)) = self.step.import_completed(proposal_hash) {
                ...
                // FIXME: We don't need to propagate the generated block if we know a higher priority block
    ...
    fn generate_seal(&mut self, height: Height, parent_hash: BlockHash) -> Seal {
        ...
        if self.is_signer_highest(&parent_hash).is_some() {
    ...
    fn proposal_generated(&mut self, sealed_block: &SealedBlock) {
        ...
        if let Some((target_priority_info, expected_parent_hash)) = self.step.generation_completed() {
    ...
    fn verify_priority_info(&mut self, height: Height, priority_info: PriorityInfo) -> Option<bool> {
        let verified_seed = self.prev_vrf_seed_of_height(height)?;
        ...
        priority_info.verify(&verified_seed, &signer_public, voting_power, &mut self.sortition_scheme).ok()
    }
