...
use super::priority_message_collector::PriorityMessageCollector;
...
struct Worker {
    ...
    priority_messages: PriorityMessageCollector,
...
impl Worker {
    ...
    fn fetch_vrf_seed_info(&self, block_hash: BlockHash) -> Option<SeedInfo> {
        ...
        Some(TendermintSealView::new(&block_seal).vrf_seed_info().unwrap())
    }
            ...
            Step::Propose => {
                ...
                if let Some(sortition_info) = self.signer_priority_message(parent_block_hash) {
                    if let TwoThirdsMajority::Lock(lock_view, locked_block_hash) = self.last_two_thirds_majority {
                        ...
                        self.priority_messages.insert(sortition_info.clone(), self.current_sortition_round());
                        self.update_sealing(parent_block_hash);
                        self.step.wait_block_generation(sortition_info, parent_block_hash);
                    }
                } else {
    ...
    fn handle_self_genearted_proposal(
        &mut self,
        proposal_hash: BlockHash,
        priority_message: PriorityMessage,
        imported_block: Bytes,
    ) {
        let vote_step = VoteStep::new(self.height, self.view, Step::Propose);
        let signature =
            self.votes.round_signature(&vote_step, &proposal_hash).expect("Proposal vote is generated before");
        self.proposal.new_highest(proposal_hash, priority_message, imported_block, signature);
        self.proposal.new_imported(proposal_hash);
    }
    ...
    fn on_imported_proposal(&mut self, proposal: &Header) {
        ...
        if proposal_is_for_current {
            self.proposal.new_imported(proposal_hash);
            if let Some((priority_message, imported_block)) = self.step.import_completed(proposal_hash) {
                cinfo!(ENGINE, "Submitting proposal block {}", proposal_hash);

                // When the proposal is imported from generated by myself, the proposal can only be registered after the block is generated.
                // FIXME: Simplify the design
                ...
                if self.step.is_propose_step_ended() {
    ...
    fn proposal_generated(&mut self, sealed_block: &SealedBlock) {
        ...
        if let Some((target_priority_message, expected_parent_hash)) = self.step.generation_completed() {
            ...
            self.step.wait_imported(target_priority_message, sealed_block.clone());
        } else {
            ctrace!(
                ENGINE,
    ...
    fn on_timeout(&mut self, token: usize) {
        ...
        self.step.mark_timed_out_if_propose_step();
        let next_step = match &self.step {
            TendermintState::Propose(inner) => {
                if let Some(wait_block_generation) = inner.get_wait_block_generation() {
                    ...
                    return
                } else if !inner.get_wait_imported().is_empty() {
                    cwarn!(ENGINE, "Propose timed out but still waiting for the blocks imported");
                    return
                }
    ...
    fn on_proposal_message(
        ...
        // This block borrows bytes
        {
            ...
            cinfo!(
                ENGINE,
                "Proposal received for ({},{})-{:?}. The priority message was created by index {}-{}",
                number,
                proposed_view,
                header_view.hash(),
                priority_message.seed_signer_idx(),
                priority_message.priority_info.sub_user_idx()
            );
