...
use super::types::{Height, Proposal, Step, View};
use super::PriorityMessage;
use crate::db;
...
pub struct PriorityMessageProjection(pub (SortitionRound, PriorityMessage));

impl PriorityMessageProjection {
    pub fn get_view(&self) -> PriorityMessageProjectionView {
        let (ref sortition_round, ref priority_message) = self.0;
        PriorityMessageProjectionView((sortition_round, priority_message))
    }
}

impl Decodable for PriorityMessageProjection {
...
pub struct PriorityMessageProjectionView<'a>(pub (&'a SortitionRound, &'a PriorityMessage));

impl Encodable for PriorityMessageProjectionView<'_> {
    fn rlp_append(&self, s: &mut RlpStream) {
        let (sortition_round, priority_message) = self.0;
        s.begin_list(2).append(sortition_round).append(priority_message);
    }
}
...
pub struct BackupView<'a> {
    ...
    pub priority_messages: &'a [PriorityMessageProjectionView<'a>],
...
pub struct BackupDataV0 {
    ...
    pub priority_messages: Vec<PriorityMessageProjection>,
...
pub fn backup(db: &dyn KeyValueDB, backup_data: BackupView) {
    ...
    s.append_list(priority_messages).append(proposal);
...
fn migrate_from_0_to_1(db: &dyn KeyValueDB) {
    ...
    let v1 = BackupDataV1 {
        ...
        step: v0.step,
        votes: v0.votes,
        priority_messages: v0.priority_messages,
    ...
    backup(db, BackupView {
        ...
        step: &v1.step,
        votes: &v1.votes,
        priority_messages: &v1.priority_messages.iter().map(|projection| projection.get_view()).collect::<Vec<_>>(),
